...
class EditKosActivity : AppCompatActivity() {
    ...
    private fun setData() {
        ...
        binding.apply {
            colomTextKos.setText(kos.namaKos)
            colomAlamat.setText("Jln. Yang Lurus Nomor 12, samping pos satpam")
            colomDaerah.setText(kos.alamat)
            colomJarak.setText("2")
            kisaranHarga.setText(kos.harga.toString())
        }
    ...
    private fun simpan() {
        ...
        viewModel.update(reqData1).observe(this) {

            when (it.state) {
                State.SUCCESS -> {
                    dismisLoading()
                    showToast("Update Kos Sukses")
                    getData()
                    finish()
                }
                State.ERROR -> {
